...
(6, 9)  
피연산자 6에 9를 곱하고, 결과 54에서 십의 자리와 일의 자리를 나눠 5, 4로 적는다.  
(5, 4)  
두 수를 더하면 9가 나온다. 구거법 방식대로 상위 자리와 하위 자리를 표기하면 9, 1이 된다. 만약 합이 8이었다면 8, 1로 적었을 거다.  
(9, 1)

이게 기본 구조다. 이걸 2x3 배열로 바꿔서 쓰면 모듈러 연산이 훨씬 단순해진다.

```python
matrix = [[6, 9], [5, 4], [9, 1]]
if abs(matrix[0][0] - matrix[2][1]) == abs(matrix[0][1] - matrix[2][0]):
    print("구거법 검산 결과 정상입니다.")
```
