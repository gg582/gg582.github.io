...
상하와 좌우의 휘도 차를 비교해 에지 방향을 고르고, 그 방향을 가로지르는 두 이웃을 평균내어 중심 색에 제한적으로 섞는다. `--aa=fxaa`는 이 판단을 화면 전체에서 한다. `--aa=adaptive`는 위 기하 분석을 통과한 픽셀에서만 같은 계산을 하며, 분석의 강도(score)와 휘도 대비가 클수록 혼합량을 조금 올린다.
...
global FXAA amount   = 0.16 + 0.10 * contrast
...
아래는 뚜껑 림을 세 배 확대한 결과다. 왼쪽부터 `off`, 전역 `fxaa`, 적응형 `adaptive`다. 가운데는 화면 전체 FXAA의 결과이고, 오른쪽은 객체-배경 실루엣과 림의 기하 변화로 판정된 픽셀만 보간한 결과다.
